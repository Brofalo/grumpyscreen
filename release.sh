...
# ASSET_NAME becomes a filename and a sed selector below, so pin its shape
# before use: letters, digits, dot, underscore, hyphen. A path separator or a
# shell metacharacter here would write outside this directory or be re-split.
case "$ASSET_NAME" in
  ""|*[!A-Za-z0-9._-]*)
    echo "ERROR: refusing ASSET_NAME with unexpected characters: $ASSET_NAME" >&2
    exit 1
    ;;
esac
